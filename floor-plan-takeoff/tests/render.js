...
(async () => {
  ...
  for (const j of jobs) {
    const dataUrl = await page.evaluate(async (j) => {
      if (!window.__doc) { const r = await fetch('http://127.0.0.1:8765/plans.pdf'); window.__doc = await pdfjsLib.getDocument({ data: new Uint8Array(await r.arrayBuffer()) }).promise; }
