...
const fs = require('fs');
(async () => {
  const jobs = JSON.parse(process.argv[2]); jobs.forEach(j => j.pdf = process.argv[3]);   // [{page, scale, crop:[x0,y0,x1,y1] fractions|null, out}]
  ...
  for (const j of jobs) {
    const dataUrl = await page.evaluate(async (j) => {
      if (!window.__doc) { const r = await fetch(j.pdf); window.__doc = await pdfjsLib.getDocument({ data: new Uint8Array(await r.arrayBuffer()) }).promise; }
