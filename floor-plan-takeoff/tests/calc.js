...
(async () => {
  ...
  fs.writeFileSync('jobs/kalinda/kalinda-takeoff.txt', out.txt); fs.writeFileSync('jobs/kalinda/kalinda-takeoff.csv', out.csv); fs.writeFileSync('jobs/kalinda/kalinda-databuild.csv', out.db);
  console.log(JSON.stringify(out.sum, null, 1)); console.log('errors:', errs.join('; ') || 'none');
  await page.click('[data-tab="takeoff"]'); await page.waitForTimeout(300); await page.screenshot({ path: 'jobs/kalinda/img/app-takeoff.png' });
  await browser.close();
