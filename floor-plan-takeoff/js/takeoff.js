...
function renderTakeoff() {
  ...
  const doorRows = S.doors.slice().sort((a, b) => cmpTag(a.tag, b.tag)).map(d => { const t2 = DOOR_TYPES[d.type] || {}, lv = leverFor(d); return `<tr><td>${esc(d.tag)}</td><td>${esc(whereStr(d))}</td><td>${esc(doorLabel(d))}</td><td class="n">${t2.noLeaf ? `${doorSpanMm(d)} w${leavesOf(d) > 1 ? ` (${leavesOf(d)} panels)` : ''}` : esc(sizeStr(d))}</td><td>${esc(d.hand || '')}</td><td>${esc(t2.noLeaf ? '' : d.leaf || '')}</td><td>${esc(t2.noLeaf ? '' : (d.colour || sp.doorColour || ''))}</td><td>${esc(lv ? (LEVER_LABEL[lv] || lv) : '')}</td><td class="n">${qtyOf(d)}</td><td>${esc([d.fire ? 'Solid core / FR' : '', d.notes].filter(Boolean).join(' · '))}</td></tr>`; }).join('');
...
function takeoffText(t) {
  ...
  for (const d of S.doors.slice().sort((a, b) => cmpTag(a.tag, b.tag))) { const t2 = DOOR_TYPES[d.type] || {}, lv = leverFor(d); L.push(pad(d.tag, 6) + pad(whereStr(d).replace(' → ', ' > '), 28) + pad(doorLabel(d), 36) + pad(t2.noLeaf ? `${doorSpanMm(d)} w${leavesOf(d) > 1 ? ` (${leavesOf(d)} panels)` : ''}` : sizeStr(d), 22) + pad(d.hand || '', 8) + pad(t2.noLeaf ? '' : (d.leaf || ''), 22) + pad(lv ? (LEVER_LABEL[lv] || lv) : '', 26) + (qtyOf(d) > 1 ? ' ×' + qtyOf(d) : '') + (d.fire ? ' · solid core/FR' : '') + (d.notes ? ' · ' + d.notes : '')); }
